...
mkdir -p "files/attack_anim/adult/valid_theta_err${valid_theta_err}/"
mkdir -p "files/attack_anim/adult/valid_theta_err${valid_theta_err}/thresh${err_thresh}/"
...
if !(test -f "${dst_fname}"); then
  ...
  mv files/online_models/adult/svm/feature/12/orig/1/*.npz \
    "files/attack_anim/adult/valid_theta_err${valid_theta_err}/thresh${err_thresh}/"
